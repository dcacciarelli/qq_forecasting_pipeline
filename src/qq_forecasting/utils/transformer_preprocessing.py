...
def get_batch(source, i, batch_size):
    ...
    x = torch.stack([item[0] for item in data])  # shape: (batch_size, input_window)
    y = torch.stack([item[1] for item in data])
    x = x.transpose(0, 1).unsqueeze(-1)  # (input_window, batch_size, 1)
    y = y.transpose(0, 1).unsqueeze(-1)  # (input_window, batch_size, 1)
    return x, y
